...
def plot(fits_image, path, name):
    ...
    colorbar.set_label(r'RMS ($\mu$Jy/Beam)', fontsize=20, color='black')
    ax.set_xlabel('RA (J2000)',size=20 )
    ax.set_ylabel('Dec (J2000)', size=20)
    ax.tick_params(axis='both', which='major', direction='in', length=8, width=1.5, labelsize=16)
    #ax.tick_params(axis='both', which='minor', direction='in', length=4, width=1, labelsize=16)
