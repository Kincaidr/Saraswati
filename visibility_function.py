...
def plot(areas,full_areas, rmsmin):
    ...
    # Original plot (blue)
    ax1.plot(rmsmin[0] * 1e3, areas[0], color='blue',label='A2631', linewidth=2)
    ax1.plot(rmsmin[1] * 1e3, areas[1], color='red',label='Zwcl2341',linewidth=2)
    ax1.set_xlabel(r'RMS (mJy/Beam)', size=22)
    ax1.set_ylabel('Area (deg²)', size=22)
    ax1.tick_params(axis='both', which='major', direction='in', length=8, width=1.5, labelsize=16)
    ...
    ax2.set_ylabel('Area Percent (%)', size=22)
