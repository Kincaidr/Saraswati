...
def scale_flux(flux, counts_freq=1400, data_freq=1283, spectral_index=-0.7):
...
def plot(cat1,cat2):
        ...
        flux1= scale_flux(flux1, counts_freq=1400, data_freq=1283, spectral_index=-0.7)
        snr1=cat1['Isl_rms']*1e3
        snr2=cat2['Isl_rms']*1e3
        mask= (flux1/snr1 > 10) & (flux2/snr2 > 10) 
        mask2= (flux1/peak1 > 0.6) & (flux1/peak1 < 1.4) & (flux2/peak2 > 0.6)  & (flux2/peak2 < 1.4) 

        table1=cat1[mask&mask2]
        table2=cat2[mask&mask2]
        ...
        plt.ylabel(r'MeerKAT $S_{T}$ [mJy]',fontsize=20)
        plt.xlabel(r'FIRST $S_{T}$ [mJy]',fontsize=20)
        ...
        axtop.axvline(np.median(flux1),linestyle='dashed',color='orange',linewidth=2)
        ...
        axright.axhline(np.median(flux1),linestyle='dashed',color='orange',linewidth=2)
        axtop.set_xticks([])
        axtop.set_yticks([])
        axright.set_xticks([])
        axright.set_yticks([])
...
if __name__ == "__main__":
    ...
    cat1=f'{path}/MeerKAT_combined.fits'
