...
def theta_min(S,N, bmaj, bmin,a,b):
    theta_min=np.sqrt(bmaj*bmin)*np.sqrt(func(a,b,S,N)-1)
    return(theta_min)

def theta_lim(S,N, sigma, bmaj, bmin, a,b):
        x=theta_min(S,N, bmaj, bmin, a,b)
...
def plot(true_sizes,sigma_high, alpha_beta_array):
    ...
    S=np.linspace(0.1,100,num=10000)
    SN=np.linspace(1,100,10000)
    x1, y1 = complementary_ecdf(true_sizes[0],bin_size=40)
    x2, y2 = complementary_ecdf(true_sizes[1],bin_size=40)
   
    S,Theta_lim= theta_lim(S,SN, sigma_high[0], bmaj, bmin, alpha_beta_array[1][0], alpha_beta_array[1][1])
    ...
    ax1.plot(x1, y1, drawstyle='steps-post', linewidth=2, label='A2631', color='blue')
    ax1.plot(x2, y2, drawstyle='steps-post', linewidth=2, label='Zwcl2341', color='red')
    # ax1.plot(bin_edges, cumy, color='blue', label='A2631', linewidth=2)
    # ax1.plot(bin_edges2, cumy1, color='red', label='Zwcl2341', linewidth=2)
    # theta_lim_arr= np.linspace(0, 200, num=10000)
    # S=np.linspace(0.01,100,num=10000)
    h1= integral_dist(median_size1(S, k=4, m=0.3),  Theta_lim)
    h2= integral_dist(median_size2(S, k=4, m=0.03),  Theta_lim)
    ...
    ax2 = axes[1]
    c1= 1/(1 - integral_dist(median_size1(S, k=4, m=0.3), Theta_lim))
    c2= 1/(1 - integral_dist(median_size2(S, k=4, m=0.03), Theta_lim))
