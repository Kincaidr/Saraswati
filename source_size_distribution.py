...
def plot(cat, bmaj, bmin, sigma_high,axs,color,alpha_beta_array):
    ...
    axs.scatter(bin_centers, median_sizes, label='Median size', marker='*',color=color_median)
    ...
    axs.plot(S, median_size(S, k=4, m=0.03), label=r"$4 \times$ Windhorst model ($m=0.03$)", color="purple")
    axs.plot(S, median_size(S, k=4, m=0.3), label=r"$4 \times$ Windhorst model ($m=0.3$)", color="grey")
    axs.set_xscale('log')
    axs.set_ylim(-2,80)
