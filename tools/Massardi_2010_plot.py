...
def Massardi_counts():
    ...
    sphero_model=np.loadtxt('tools/spherodial_galaxy.txt')
    ...
    S_sphero=(10**sphero_model[:,0])*1e3 # Convert to mJy
    spheroidal_galaxy=10**sphero_model[:,1]
    sphero_func = interp1d(S_sphero, spheroidal_galaxy, bounds_error=False, fill_value=0)
    ...
    new_grid = np.linspace(1e-4, 100, 1000000)  # 0.01 mJy to 100 mJy
    spheroidal_galaxy_interp = sphero_func(new_grid)
    ...
    total_N=FSRQ_interp + BL_Lac_interp + steep_spectrum_interp + Spirals_interp + Starburst_interp#+spheroidal_galaxy_interp
