...
def get_data(file1):
    ...
    #comp1_corr=table1[:,2]
    err1=table1[:,2]
    return(bin1,comp1,err1)

def plot(bins,comps,errs):
    plt.errorbar(bins[0],comps[0],yerr=errs[0], label='A2631', color='blue',linewidth=3)
    #plt.errorbar(bins[0],comp_corrs[0],yerr=errs[0], color='blue',linestyle='--')
    plt.errorbar(bins[1],comps[1],yerr=errs[1], label='ZwCL2341', color='red',linewidth=3)
    #plt.errorbar(bins[1],comp_corrs[1],yerr=errs[1],linestyle='--', color='red')
    plt.xlabel(r'Log Flux density [mJy]',size=18)
    plt.ylabel('Detected Fraction',size=18)
    plt.tick_params(axis='both', which='major', labelsize=14, length=8, width=1.5)  # Increase size of major tick labels
    plt.tick_params(axis='both', which='minor', labelsize=14, length=4, width=1)
...
if "__main__":
    ...
    for name in names:
        ...
        bin,comp,err=get_data(file1)
        bins.append(bin)
        comps.append(comp)
        errs.append(err)
        #comp_corrs.append(comp_corr)
    plot(bins,comps,errs)