...
def completeness_file(output_filename,inj_sources, no_simulations, output_file ):
    ...
    error=(np.sqrt(rec_counts)/rec_counts)**2
...
def get_data(visibility_correction_file,  completeness):    
    ...
    new_visib_scale=f(comp_bin)
    comp_corr=comp*new_visib_scale
    breakpoint()
    with open('zwcl_visib_correction.txt', "w") as file:
        for bin, co, scale, err in zip(comp_bin,comp,comp_corr, comp_err ):
            file.write(f"{bin} {co} {scale} {err}\n")
...
if "__name__":
    ...
    completeness=completeness_file(completeness_filename,inj_sources, no_simulations,output_file )
    bin,comp,visib_scale,err=get_data(visibility_correction_file,  completeness)
